...
class RedfishUtils(object):
    ...
    def _find_chassis_resource(self):
        ...
        if 'Chassis' not in data:
            ...
            for member in data[u'Members']:
                chassis_list.append(member[u'@odata.id'])
            self.chassis_uris = chassis_list
            return {'ret': True}
    ...
    def _find_managers_resource(self):
        ...
        if 'Managers' not in data:
            ...
            data = response['data']
            for member in data[u'Members']:
                managers_list.append(member[u'@odata.id'])
            self.manager_uris = managers_list
            # for commands that only work on a single manager
            self.manager_uri = managers_list[0] if len(managers_list) else None
            return {'ret': True}
